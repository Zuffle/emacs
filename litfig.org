...
(defconst WORKENV (string-equal user-login-name "eparjus"))

; Verilog indenting changes
(let ((ind 2))
  (gsetq verilog-indent-level ind
	 verilog-indent-level-directive ind
	 verilog-indent-level-module ind
	 verilog-indent-level-behavioral ind
	 verilog-indent-level-declaration ind
	 ))

(gsetq remote-file-name-inhibit-cache nil)
(gsetq vc-ignore-dir-regexp (format "%s\\|%s" vc-ignore-dir-regexp tramp-file-name-regexp))
(gsetq tramp-verbose 1)
#+end_src
...
(when WORKENV 
  (server-start)
  ;; (gsetq server-use-tcp t)
  (gsetq server-port 50782)
  (gsetq server-auth-key "q9xj@<_i5t-kw-o;^,D:!]9]\,{aKx3F\"WDH.e8K,k$2Du@(igwzj'3dv1dN7c$S"))
#+end_src
...
(general-setq display-line-numbers-widen t)
